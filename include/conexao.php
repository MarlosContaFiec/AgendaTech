...
try {
    ...
    error_log("❌ Erro: Arquivo .env não encontrado ou corrompido.");
    throw new Exception("Configurações do sistema ausentes.");
}
...
// Loop de conexão (Ajustado para Docker)
while ($tentativa < $maxTentativas) {
    try {
        ...
        $tentativa++;
        error_log("⚠️ Tentativa de conexão $tentativa/$maxTentativas falhou...");
        sleep(2);
    }
}

if (!$pdo) {
    error_log("❌ Erro Crítico: Banco de dados não respondeu após $maxTentativas tentativas.");
    throw new Exception("O serviço de banco de dados está temporariamente indisponível.");
}
...
try {
    ...
    // --- CRIAÇÃO DE ÍNDICES ---
    // Agrupados em um loop para evitar repetição de try/catch
    $indices = [
        "idx_ag_cliente"      => "CREATE INDEX idx_ag_cliente ON agendamento (cliente_id)",
        "idx_ag_servico"      => "CREATE INDEX idx_ag_servico ON agendamento (servico_id)",
        "idx_msg_empresa"     => "CREATE INDEX idx_msg_empresa ON mensagem (empresa_id)",
        "idx_msg_cliente"     => "CREATE INDEX idx_msg_cliente ON mensagem (cliente_id)",
        "idx_av_agendamento"  => "CREATE INDEX idx_av_agendamento ON avaliacao (agendamento_id)",
        "idx_servico_empresa" => "CREATE INDEX idx_servico_empresa ON servico (empresa_id)",
        "idx_tags_empresa"    => "CREATE INDEX idx_tags_empresa ON tags (empresa_id)",
        "idx_regras_empresa"  => "CREATE INDEX idx_regras_empresa ON regras (empresa_id)"
    ];
    ...
    foreach ($indices as $nome => $sql) {
        try {
            ...
            // Ignora se o índice já existir (Erro 42S21 ou 1061)
        }
    }

} catch (PDOException $e) {
    error_log("❌ Erro SQL: " . $e->getMessage());
    throw new Exception("Falha na estrutura do banco de dados.");
} catch (\Exception $e) {
