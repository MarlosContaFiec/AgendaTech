  ...
  return (
    <div className="w-full max-w-[500px] bg-surface border border-line rounded-[20px] px-9 py-8 shadow-2xl animate-[fadeUp_.3s_ease]">
      ...
      <div className="flex items-center gap-2.5 mb-5">
        <div className={"w-[34px] h-[34px] rounded-[10px] flex items-center justify-center text-[1.1rem] flex-shrink-0 " + (emp ? "bg-gradient-to-br from-purple to-neon" : "bg-gradient-to-br from-purple to-neon")}>{emp ? "🏢" : "👤"}</div>
        <div>
      ...
      <div className="flex flex-col gap-3">
        {emp ? (
          <>
            ...
            <Input label="CNPJ" icon="🪪" placeholder="00.000.000/0001-00" value={f.cnpj} onChange={v => upd("cnpj", maskCNPJ(v.target.value))} error={e.cnpj} required />
            <div className="grid grid-cols-2 gap-3">
              <Input label="Telefone" icon="📱" placeholder="(11) 99999-0000" value={f.telefone} onChange={v => upd("telefone", maskPhone(v.target.value))} />
              <Input label="CEP" icon="📍" placeholder="13330-000" value={f.cep} onChange={v => upd("cep", v.target.value)} />
            </div>
          </>
        ) : (
          <>
            <Input label="Nome Completo" icon="✏️" placeholder="Maria Silva Santos" value={f.nome} onChange={v => upd("nome", v.target.value)} error={e.nome} required />
            <div className="grid grid-cols-2 gap-3">
              <Input label="CPF" icon="🪪" placeholder="000.000.000-00" value={f.cpf} onChange={v => upd("cpf", maskCPF(v.target.value))} error={e.cpf} />
              <Input label="Telefone" icon="📱" placeholder="(11) 99999-0000" value={f.telefone} onChange={v => upd("telefone", maskPhone(v.target.value))} />
            </div>
            <Input label="Data de Nascimento" type="date" value={f.nascimento} onChange={v => upd("nascimento", v.target.value)} />
          </>
        )}
        <Input label="E-mail" icon="✉️" type="email" placeholder="seu@email.com" value={f.email} onChange={v => upd("email", v.target.value)} error={e.email} required />
        <InputSenha label="Senha" placeholder="Mínimo 6 caracteres" value={f.senha} onChange={v => upd("senha", v.target.value)} />
        <PasswordStrength senha={f.senha} />
        {e.senha && <p className="text-[0.72rem] text-danger flex items-center gap-1"><span>!</span>{e.senha}</p>}
        <InputSenha label="Confirmar Senha" placeholder="Repita a senha" value={f.confirma} onChange={v => upd("confirma", v.target.value)} />
        ...
        <Button loading={loading} disabled={loading} onClick={submeter} className="w-full">Criar Conta →</Button>
      </div>
      <div className="mt-4"><button onClick={onVoltar} className="bg-none border-none cursor-pointer text-muted text-[0.82rem] font-medium hover:text-purple transition-colors">← Voltar</button></div>
    </div>
  );
}
