...
async function testHealth() {
  ...
  ok('body.ok === true', r.body.ok === true);
}
...
async function testTags() {
  ...
  ok('Tag dup → 409',
    (await api('POST', '/api/tags', { nome: 'tag_teste', label: 'Dup', cor: '#000', aceita_agendamento: true }, ctx.tE)).status === 409);
}
...
async function testCliente() {
  section('CLIENTE — Perfil & Calendário');
