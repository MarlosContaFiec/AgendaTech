...
export default function TelaCadastro({ tipo, onConcluir, onVoltar }) {
  ...
  const [f, setF] = useState(isEmp
    ? { razao:'', fantasia:'', cnpj:'', email:'', senha:'', confirma:'' }
  ...
  async function submeter() {
    ...
    const res = isEmp
      ? await cadastrarEmpresa({ razao_social:f.razao.trim(), nome_fantasia:f.fantasia.trim(), cnpj:f.cnpj, email:f.email.trim().toLowerCase(), senha:f.senha })
  ...
  return (
    <div className="w-full max-w-[500px] bg-tb-surface border border-tb-border rounded-[20px] py-9 px-9 pb-8 shadow-[0_24px_64px_rgba(0,0,0,0.5),0_0_0_1px_rgba(79,140,255,0.06)] animate-fadeUp">
      ...
      <div className="flex items-center gap-2.5 mb-[22px]">
        <div className={'w-[34px] h-[34px] rounded-[10px] flex items-center justify-center text-[1.1rem] flex-shrink-0 ' + (isEmp ? 'bg-gradient-to-br from-tb-violet to-pink-500' : 'bg-gradient-to-br from-tb-accent to-indigo-500')}>{isEmp ? '🏢' : '👤'}</div>
        <div>
          <h2 className="text-[1.1rem] font-black text-tb-text tracking-tight">Criar conta — {isEmp ? 'Empresa' : 'Cliente'}</h2>
          <p className="text-[0.75rem] text-tb-muted2">{isEmp ? 'Dados do estabelecimento' : 'Preencha seus dados'}</p>
        </div>
      </div>
        ...
        {isEmp ? (
          <>
            <div className="grid grid-cols-2 gap-3">
              <Input label="Razão Social" placeholder="Empresa Ltda." value={f.razao} onChange={v=>upd('razao',v.target.value)} erro={e.razao} obrigatorio />
              <Input label="Nome Fantasia" placeholder="Meu Negócio" value={f.fantasia} onChange={v=>upd('fantasia',v.target.value)} erro={e.fantasia} obrigatorio />
            </div>
            <Input label="CNPJ" icone="🪪" placeholder="00.000.000/0001-00" value={f.cnpj} onChange={v=>upd('cnpj',maskCNPJ(v.target.value))} erro={e.cnpj} obrigatorio />
          </>
        ) : (
          <>
            <Input label="Nome Completo" placeholder="Ex: Maria Silva Santos" value={f.nome} onChange={v=>upd('nome',v.target.value)} erro={e.nome} obrigatorio />
            <Input label="CPF" placeholder="000.000.000-00" value={f.cpf} onChange={v=>upd('cpf',maskCPF(v.target.value))} erro={e.cpf} obrigatorio />
          </>
        )}
        <Input label="E-mail" icone={ <span className='material-icons-outlined' style={{ fontSize: 25 }}>mail</span> } type="email" placeholder="seu@email.com" value={f.email} onChange={v=>upd('email',v.target.value)} erro={e.email} obrigatorio />
